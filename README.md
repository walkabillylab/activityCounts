...
function counts. Also, For sampleXYZ data, the first column is X, second is Y, and the third column is the Z axis, therefore when calling the function, specify the columns in the argument section.
...
raw data are stored in the second, the third, and the forth columns,
...
The second column has the x-axis data, the third column has the y-axis data,
and the fourth column has the z-axis data. Therefore, assuming the sampling
frequency is 50 Hz, we call the function like this:
  
  
  ``` r
calculated_output <- counts(data = your_raw_data, hertz = 50, x_axis = 2, y_axis = 3, z_axis = 4)
```
